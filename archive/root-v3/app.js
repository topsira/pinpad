...
boardTitle.addEventListener("input", (event) => {
  ...
  saveSettings();
  document.title = `${event.target.value.trim() || DEFAULT_SETTINGS.boardTitle} - pinpad`;
});
...
resetButton.addEventListener("click", () => {
  const confirmed = confirm("Reset all notes on this board? This cannot be undone.");
  if (!confirmed) return;
